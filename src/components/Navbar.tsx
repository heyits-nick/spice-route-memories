...
const Navbar = () => {
  ...
  return <nav className="bg-spice-cream border-b border-spice-brown/10 sticky top-0 z-50">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-center space-x-4">
              ...
              <button onClick={() => navigate("/cart")} className="relative p-2 rounded-full bg-spice-turmeric hover:bg-spice-turmeric/90 text-black">
                ...
                {totalItems > 0 && <span className="absolute -top-1 -right-1 bg-spice-red text-white rounded-full w-5 h-5 flex items-center justify-center text-xs">
                    {totalItems}
                  </span>}
              </button>
            </div>
      ...
      {/* Mobile menu */}
      {isMenuOpen && <div className="md:hidden bg-white border-t border-spice-brown/10">
